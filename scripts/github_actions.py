...
class GitHub:
    ...
    def run(self, *args):
        mutating = args[:2] in (("workflow", "run"), ("run", "rerun"))
        uncertain = (
...
def plan_run(client, pr, mode):
    ...
    if mode in ("required", "additional"):
        return (
            [
                "workflow",
                "run",
    ...
    runs = client.read_json(
        ...
        "1",
        "--json",
        "databaseId,status,url,headSha,headBranch",
    )
    if (
        not runs
...
def launch(client, pr, mode, dry_run=False, yes=False):
    ...
    # gh dispatches by branch; GitHub resolves its head at acceptance. Never retry launches.
